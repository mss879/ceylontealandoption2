...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
    ...
    const portalContent = (
        ...
        >
            <div
                ref={preLayersRef}
                className="sm-prelayers absolute top-0 right-0 bottom-0 pointer-events-none z-[105]"
                aria-hidden="true"
            >
                {(() => {
    ...
    return (
        <div className={`sm-scope inline-block ${className || ''}`}>
            <button
                ref={toggleBtnRef}
                className={`sm-toggle relative inline-flex items-center gap-[0.3rem] bg-transparent border-0 cursor-pointer font-medium leading-none overflow-visible pointer-events-auto py-2 px-5 m-[-0.5rem_-1.25rem] ${open ? 'text-black z-[120]' : 'text-current z-50'
                    }`}
            ...
            >
                <span
                    ref={textWrapRef}
                    className="sm-toggle-textWrap relative inline-block h-[1em] overflow-hidden whitespace-nowrap w-[var(--sm-toggle-width,auto)] min-w-[var(--sm-toggle-width,auto)] pointer-events-none"
                    aria-hidden="true"
                >
                ...
                <span
                    ref={iconRef}
                    className="sm-icon relative w-[14px] h-[14px] shrink-0 inline-flex items-center justify-center [will-change:transform] pointer-events-none"
                    aria-hidden="true"
                >
                    <span
                        ref={plusHRef}
            ...
            <style>{`
        .sm-scope .sm-toggle { position: relative; display: inline-flex; align-items: center; gap: 0.3rem; background: transparent; border: none; cursor: pointer; color: inherit; font-weight: 700; line-height: 1; overflow: visible; }
